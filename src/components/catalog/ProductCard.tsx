...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const handleViewDetails = (e?: React.MouseEvent) => {
    ...
    if (onViewDetails) {
      ...
      // Se não conseguir, não redirecionar para evitar erro
      try {
        ...
        if (isSubdomain && subdomain) {
          ...
          console.log('🔄 ProductCard - Redirecionando para subdomínio:', `/produto/${product.id}`);
          ...
          if (storeSlug) {
            console.log('🔄 ProductCard - Redirecionando para URL padrão:', `/catalog/${storeSlug}/produto/${product.id}`);
            window.location.href = `/catalog/${storeSlug}/produto/${product.id}`;
          } else {
            console.warn('⚠️ ProductCard - Não foi possível redirecionar: falta store_slug. Use onViewDetails prop.');
            // Não fazer nada - deixar o componente pai lidar com isso
          }
        }
      } catch (error) {
        console.error('❌ ProductCard - Erro ao gerar URL:', error);
        // Não fazer nada - deixar o componente pai lidar com isso
      }
    }
  };
