...
const EnhancedCheckout: React.FC<EnhancedCheckoutProps> = ({
  ...
  console.log("EnhancedCheckout: config carregado:", {
    ...
    checkoutUpsellEnabled: config?.store_settings?.checkout_upsell_enabled,
  ...
  const generateWhatsAppMessage = (data: CheckoutForm, order?: any) => {
    ...
    message += `🚚 *Entrega:* ${
      config?.shipping_methods.find((m) => m.id === data.shippingMethod)?.name
    }\n`;
    message += `💳 *Pagamento:* ${
      ...
      <Form {...form}>
        ...
        >
          ...
          <div className="space-y-6">
            {/* Order Bumps */}
            {config?.store_settings.checkout_upsell_enabled &&
            ...
            {/* Trust Badges */}
            <TrustBadges
              enabled={config?.store_settings.trust_badges_enabled}
            />
          </div>
        </form>
      </Form>
