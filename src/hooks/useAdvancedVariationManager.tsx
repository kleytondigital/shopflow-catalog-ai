...
export const useAdvancedVariationManager = (
  ...
  const managedVariations = useMemo(() => {
    // Sempre usar as variações externas se existirem
    return variations;
  }, [variations]);
  ...
  const createAllCombinations = useCallback(
    (colors: string[], sizes: string[], materials?: string[]) => {
          ...
          if (materials && materials.length > 0) {
            materials.forEach((material) => {
              combinations.push({
                ...
                price_adjustment: 0,
                is_active: true,
                sku: `${color.toLowerCase()}-${size.toLowerCase()}-${material.toLowerCase()}`.replace(
                  /\s+/g,
                  "-"
                ),
      ...
      // Sempre usar o callback para atualizar as variações
  ...
  const getStatistics = useCallback(() => {
    ...
    return {
      ...
      averagePrice:
        current.length > 0
          ? current.reduce((sum, v) => sum + (v.price_adjustment || 0), 0) /
            current.length
          : 0,
    };
  }, [managedVariations]);
  ...
  const updateVariation = useCallback(
    (index: number, updates: Partial<ProductVariation>) => {
      const current = managedVariations;
